...
def test_dynamic_prompt_builder_injects_only_selected_contracts() -> None:
    ...
    assert '# Learning requests' in prompt
    assert '# Current research' in prompt
    assert 'Deliver a tutorial' in prompt
    assert '# Decision and planning requests' not in prompt
    assert 'Deliver a decision brief' not in prompt
...
def test_blocking_request_issue_is_disclosed_as_authoritative_runtime_state() -> None:
    ...
    assert '# Request quality check' in prompt
    assert '# Clarification required' in prompt
    assert '## Request Assessment [AUTHORITATIVE]' in prompt
    assert 'mathematical_inconsistency' in prompt
    assert 'Interaction need: blocking' in prompt

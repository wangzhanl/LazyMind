...
func (e *DefaultEngine) CreateSource(ctx context.Context, req CreateSourceRequest) (CreateSourceResponse, error) {
	...
	jobIDs, jobErrors := e.triggerInitialSyncs(ctx, collectBindings(prepared))
	operation.Status = OperationStatusSucceeded
	if len(jobErrors) > 0 {
		operation.Status = OperationStatusSucceededWithWarning
		operation.Warning = jobErrorsJSON(jobErrors)
	}
	...
	return CreateSourceResponse{Source: sourceToResponse(source), Bindings: bindingsToResponse(collectBindings(prepared)), JobIDs: jobIDs, JobErrors: jobErrors}, nil
}

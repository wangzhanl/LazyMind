...
func TestInitialRootsReturnDriveAndWikiVirtualBranches(t *testing.T) {
	...
	for _, item := range page.Items {
		if item.Bindable || item.BindingTargetType != "" || item.BindingTargetRef != "" {
			t.Fatalf("virtual root must not be bindable, got %+v", item)
		}
	}
...
func TestDriveFolderAndWikiNodeExposeSemanticBindingTargets(t *testing.T) {
	t.Parallel()
	...
	if len(drive.Items) != 1 || drive.Items[0].BindingTargetType != TargetTypeDriveFolder || drive.Items[0].BindingTargetRef != "drive:folder-root" {
		t.Fatalf("drive root should expose drive_folder target, got %+v", drive.Items)
	}
